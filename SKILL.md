...
**年龄对营养的影响：**
- 35岁+：蛋白质目标提升至 2.0–2.2g/kg（肌肉蛋白合成率下降，需更高刺激）
- 40岁+：亮氨酸阈值提高（每餐需3–4g才能触发MPS），建议每餐≥40g蛋白质
...
**示例：训练18:30–21:00，睡眠23:00**
- 训练前正餐：约16:30–17:00（碳水+蛋白，低脂低纤维）
- 训练前补给（可选）：18:00–18:15（香蕉/能量棒）
- **训练后黄金窗口：21:00–21:30** — 蛋白+碳水，是当天最重要的饮食时机
- 睡前：22:30 — 酪蛋白/希腊酸奶
...
| 节点 | 蛋白量 | 碳水量 | 脂肪 | 举例食物 |
|---|---|---|---|---|
| 训练前90–120min正餐 | 20–40g | 0.5–1g/kg（40–80g） | 低 | 米饭1.5碗+鸡胸150g；面条+鸡蛋 |
| 训练前30–45min（可选） | — | 15–30g | 极低 | 香蕉1根；白面包1片 |
| **训练后0–30min（最重要）** | **0.4g/kg（约30g）** | **0.5–1g/kg（40–80g）** | 极低 | 乳清1勺+香蕉；鸡蛋+米饭 |
| 睡前30–60min | 30–40g | 少 | 低 | 希腊酸奶200g；牛奶300ml+酪蛋白 |

**晚间训练特别说明（训练结束>21:00）：**
- 训练后餐不应过量——蛋白优先，碳水适量（0.5g/kg即可），避免睡前过饱
- 若训练结束至睡眠<2小时：用液态食物（乳清蛋白+牛奶）代替固体正餐，更快消化
- 晚间高强度训练后皮质醇升高——确保睡前摄入酪蛋白，对抗夜间肌肉分解
...
```
=== 营养建议 ===
姓名: [name] | 体重: [weight]kg | 年龄: [age] | 阶段: [phase]
...
Step A: 遍历【待研究问题】中所有🔍条目
Step B: 按优先级排序，高优先级先处理
Step C: 对每个问题执行网络研究
  → 使用搜索关键词在权威来源查找
  → 权威来源优先级：PubMed/系统综述 > 知名专家著作 > 专业博客
  → 找到后用 web-distill 标准提取核心内容
Step D: 将研究结果追加到对应的 references/ 知识文件
  → 格式：## 自更新补充 — [问题标题]（来源: [URL/书名], 日期: [YYYY-MM-DD]）
  → 内容：核心数字/协议/判断框架（不要故事，不要铺垫）
Step E: 从快照【待研究问题】中移除已研究的条目
Step F: 告知用户：「已自动研究并更新 [X] 个问题：[列表]」
Step G: 更新档案快照版本号（+1）
```
