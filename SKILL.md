...
1. **跳过 Step 1**（数据采集）
2. 自动解析快照中的所有字段
3. 重新运行 Step 2 诊断（用快照数据）
4. 询问："你好 [姓名]，档案已加载。本次想做什么？（更新测试数据 / 获取新计划 / 伤病咨询）"
5. 如快照有缺失字段（⬜标记），在合适时机追问，但不强迫用户立即填写
...
本体系设计为可持续进化，以下情况下**主动告知用户体系存在更新**：

1. 用户遇到体系当前无法处理的情况 → 在输出末尾标注 `[知识边界: 建议补充X模块]`
2. 诊断数据不足以精确处方 → 明确标注 `confidence=low`，列出补充后能精准化的项目
3. 用户档案v3以上时 → 每次更新档案时输出"体系已根据你[N]次训练记录累积识别到的模式"
