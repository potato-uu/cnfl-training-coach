...
16 个覆盖运动/位置/经验/伤病/赛季阶段的典型 case。
...
**Must have：**
- `current_phase = preseason`（45天 < 56天）
- 诊断垂直跳45cm低于篮球基准55cm，判定为核心缺陷
- `posterior_chain_gap = moderate`（RDL/后蹲 = 70/110 = 63.6%，在55%-70%区间）
- P0或P1包含垂直跳/弹跳开发
- 引用knowledge-basketball.md或篮球专项内容

**Must not have：**
- French Contrast（训练年限2年 < 3年）
- 深度跳（训练年限 < 3年）

**Red flags：**
- 如果只用橄榄球基准（40码）而非篮球基准（30m + 垂直跳）→ 运动专项路由失败

---

## Case 12: 篮球前锋 — 力量主导速度滞后
...
训练年限: 4年，训练频次: 5天/周
训练时间段: 18:00–20:30
设备: 杠铃 / 哑铃 / 跳箱 / 阻力带 / GHD / 单杠

力量: 后蹲160 / 前蹲130 / RDL120 / 单腿RDL40
速度: 30m冲刺4.9秒 / 5-10-5变向5.5秒
跳跃: 垂直跳50cm
上肢: 卧推100 / 引体+20kg

伤病: 右膝髌腱不适（跳跃时轻微痛，VAS 3分）
赛季开始: 90天后
```

**Must have：**
- `current_phase = offseason`（90天 > 56天）
- `strength_to_speed = strength_dominant`（深蹲/体重 = 160/88 = 1.82x ≥ 1.5，30m冲刺4.9秒慢于基准4.4秒）
- `agility_rating = poor`（5-10-5 = 5.5秒 > 基准5.0秒 + 0.5秒容差）
- `injury_flag: knee` 触发（髌腱不适）
- P0包含膝盖/髌腱康复或管理
- 可开French Contrast（训练年限4年 ≥ 3年）

**Must not have：**
- 在髌腱不适情况下安排大量深蹲跳/高强度增强式

**Red flags：**
- 如果忽略髌腱不适直接开大量增强式 → 伤病优先级违反
- 如果把 strength_dominant 误判为 balanced（深蹲/体重 1.82x + 30m 4.9s 明显不匹配）→ 力量-速度转化诊断错误

---

## Case 13: 排球主攻 — 肩膀+弹跳双缺陷
...
训练年限: 3年，训练频次: 5天/周
训练时间段: 18:30–21:00
设备: 杠铃 / 哑铃 / 跳箱 / 阻力带 / 单杠 / 绳索

力量: 后蹲120 / RDL80 / 单腿RDL30
速度: 30m冲刺4.7秒
跳跃: 垂直跳52cm
上肢: 卧推75 / 引体10个

伤病: 右肩扣球后疼痛（2周了，外展90°时痛）
赛季开始: 70天后
```

**Must have：**
- 诊断垂直跳52cm低于主攻基准60cm，判定为核心缺陷
- `posterior_chain_gap = moderate`（RDL/后蹲 = 80/120 = 66.7%，在55%-70%区间）
- `unilateral_gap = moderate`（单腿RDL/后蹲 = 30/120 = 25%，在20%-35%区间）
- `injury_flag: shoulder` 触发（右肩疼痛）
- P0为肩膀康复
- 引用排球专项内容（knowledge-volleyball.md）

**Must not have：**
- 过头推举或大量扣球训练

**Red flags：**
- 如果 P0 是垂直跳而非肩膀康复 → 伤病优先级应始终高于性能缺陷
- 如果用篮球基准（55cm）而非排球主攻基准（60cm）→ 运动专项路由失败
...
**Must have：**
- 数据大量缺失，标记 `insufficient_data`（缺失RDL、单腿RDL、速度测试等）
- 垂直跳40cm低于自由人基准50cm，判定为缺陷
- `current_phase = preseason`（30天 < 56天）
- 深蹲/体重 = 75/65 = 1.15x，低于排球基准1.3x
- 优先级考虑变向能力（自由人核心能力）
- 输出通用入门方案 + 补测清单

**Must not have：**
- 高翻、French Contrast（训练年限1年）
- 深度跳

**Red flags：**
- 如果给 1 年训练年限的人开复杂动作模式（高翻/抓举/深度跳）→ 训练年限适配违反
- 如果忽略数据缺失直接给完整诊断而非标记 insufficient_data → 失败处理违反
...
训练年限: 4年，训练频次: 6天/周
训练时间段: 06:00–08:00
设备: 杠铃 / 哑铃 / 跳箱 / 阻力带 / 单杠 / GHD

力量: 后蹲140 / 前蹲120 / 高翻90 / RDL110 / 单腿RDL50
速度: 30m冲刺4.2秒 / 10m加速2.0秒
跳跃: 垂直跳62cm / 立定跳远2.75m
上肢: 卧推85 / 引体15个
...
**Must have：**
- 30m冲刺4.2秒慢于4.0秒基准；10m加速2.0秒 > 1.85秒基准，诊断加速阶段为主要短板
- 深蹲/体重 = 140/74 = 1.89x，力量充足
- `current_phase = offseason`（100天 > 56天）
- 引用田径专项内容（knowledge-track-field.md）
- `posterior_chain_gap = acceptable`（RDL/后蹲 = 110/140 = 78.6% ≥ 70%）
- P0/P1包含加速阶段训练或冲刺力学
- `injury_flag: hamstring` 触发（历史腘绳肌拉伤，即使已恢复）

**Must not have：**
- 忽略腘绳肌拉伤史（即使已恢复，应在预防训练中予以考虑）

**Red flags：**
- 如果把 30m 4.2s 评为"严重慢" → 错（距基准 4.0s 仅差 0.2s，10m 加速才是主要短板）
- 如果完全忽略已恢复的腘绳肌拉伤史 → 伤病标记扫描应仍触发 hamstring flag

---

## Case 16: 田径跳远 — 力量不足弹性好
...
**Must have：**
- 深蹲/体重 = 80/68 = 1.18x，远低于田径基准1.6x，判定力量严重不足
- `strength_to_speed = speed_dominant`（力量低但垂直跳/立定跳不错）
- `posterior_chain_gap = acceptable`（RDL/后蹲 = 60/80 = 75% ≥ 70%）
- `current_phase = offseason`（120天 > 56天）
- P0应为最大力量发展
- 引用田径专项内容（knowledge-track-field.md）

**Must not have：**
- French Contrast或深度跳（训练年限2年 < 3年）
- 忽略力量严重不足的问题（垂直跳表现不错不代表力量基础充足）

**Red flags：**
- 如果因为垂直跳 58cm 不错就判定 balanced → 错（深蹲/体重 1.18x 远低于 1.6x 基准，力量是瓶颈）
- 如果 P0 是爆发力训练而非最大力量 → offseason + 力量严重不足应先补力量基础
